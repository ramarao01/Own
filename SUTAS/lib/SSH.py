...
class SSH(object):
    ...
    def connect(self):
        ...
        try:
            ...
            error_message = ("Failed to SSH using %s:%s@%s. %s" %
                             (self.username, self.password, self.hostname, e.args[0]))
            exc_info = sys.exc_info()
            reraise(exc_info[0], exc_info[0](error_message, e.args[1]), exc_info[2])
    ...
    def command_exec(self, command, timeout=CONNECTION_TIMEOUT, sudo=False,
        ...
        if sudo and not (sudo_password or self.password):
            ...
            command = 'echo %s | sudo -S %s' % (sudo_password or self.password,
                                                command)
        inp, out, err = self.client.exec_command(command,
                                                 bufsize=bufsize,
                                                 timeout=timeout)
    ...
    def check_output(self, command, timeout=None, sudo=False,
        ...
        return_code, stdout, stderr = (
            self.exec_command(command, timeout=timeout, sudo=sudo,
